...
. "$repoRoot\setup-scripts\commands\wizard.ps1"
...
$result = Test-VpmPackageVersion -PackageName "adjerry91.vrcft.templates" -Version "latest" -ScriptDir $repoRoot
...
$result = Test-VpmPackageVersion -PackageName "adjerry91.vrcft.templates" -Version "6.8.0" -ScriptDir $repoRoot
Write-Host "Valid: $($result.Valid) - $($result.Message)" -ForegroundColor $(if ($result.Valid) { "Green" } else { "Red" })

# Test 3: non-existing version
Write-Host "`n[Test 3] adjerry91.vrcft.templates @ 99.99.99" -ForegroundColor Yellow
$result = Test-VpmPackageVersion -PackageName "adjerry91.vrcft.templates" -Version "99.99.99" -ScriptDir $repoRoot
Write-Host "Valid: $($result.Valid) - $($result.Message)" -ForegroundColor $(if ($result.Valid) { "Green" } else { "Red" })

# Test 4: official VRChat package
Write-Host "`n[Test 4] com.vrchat.avatars @ latest" -ForegroundColor Yellow
$result = Test-VpmPackageVersion -PackageName "com.vrchat.avatars" -Version "latest" -ScriptDir $repoRoot
Write-Host "Valid: $($result.Valid) - $($result.Message)" -ForegroundColor $(if ($result.Valid) { "Green" } else { "Red" })
